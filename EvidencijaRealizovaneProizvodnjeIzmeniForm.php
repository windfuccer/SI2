...
$StariIdProizvonjeZaIzmenu=$_POST['IdProizvodnje'];
...
// KONEKTOVANJE NA BAZU
	...
	$ProizvodjacObject->UcitajKolekcijuSvihProizvodjaca();
	$KolekcijaZapisa= $ProizvodjacObject->Kolekcija;
	$UkupanBrojZapisa= $ProizvodjacObject->BrojZapisa;

	// PREUZIMANJE STARIH VREDNOSTI ZA IZABRANOG STUDENTA
	require "klase/DBEvidencijaRealizovaneProizvodnje.php";
	$EvidencijaObject = new DBEvidencijaRealizovaneProizvodnje($KonekcijaObject, 'EvidencijaRealizacijeProizvodnje');
	$EvidencijaObject->UcitajEvidencijuPoIdProizvodnje($StariIdProizvonjeZaIzmenu);
	...
	if ($UkupanBrojZapisaEvidencija>0) 
	{
		...
		$StariIdProizvodnje = $EvidencijaObject->DajVrednostPoRednomBrojuZapisaPoRBPolja ($KolekcijaZapisaEvidencija, $row, 0);
		$StariNazivProizvodnje =$EvidencijaObject->DajVrednostPoRednomBrojuZapisaPoRBPolja ($KolekcijaZapisaEvidencija, $row, 1);//mysql_result($result,$row,"REGISTARSKIBROJ");
		$StaroMestoProizvodnje=$EvidencijaObject->DajVrednostPoRednomBrojuZapisaPoRBPolja ($KolekcijaZapisaEvidencija, $row, 2);
		$StariBrojProizvodnihObjekata=$EvidencijaObject->DajVrednostPoRednomBrojuZapisaPoRBPolja ($KolekcijaZapisaEvidencija, $row, 3);
		$StariIdProizvodjaca=$EvidencijaObject->DajVrednostPoRednomBrojuZapisaPoRBPolja ($KolekcijaZapisaEvidencija, $row, 4);
...
<meta charset="UTF-8">
<head>
<title>Евиденција реализоване проиводње</title>
<meta charset="UTF-8">
...
<?php include 'delovi/desnoEvidencijaRealizovaneProizvodnjeIzmeniForm.php';?>
</td>
